...
#' cell_id()
cell_id <- function() {
  old <- get_arcET_global(which = "cell_index", default = 0)
  arcET_global$cell_index <- old + 1
...
#' @export
track_id <- function() {
  old <- get_arcET_global(which = "track_index", default = 0)
  arcET_global$track_index <- old + 1
...
#' @export
sector_id <- function() {
  old <- get_arcET_global(which = "sector_index", default = 0)
  arcET_global$sector_index <- old + 1
  paste0("SectorID_", old + 1)
}

#' @rdname IDs
#' @export
reset_ids <- function(which = c("all", "CellID", "TrackID", "SectorID")) {
  ...
  if (which == "all") {
    ...
    if (which == "CellID") {
      ...
      set_arcET_global(track_index = 0)
    } else {
      set_arcET_global(sector_index = 0)
    }
  }
...
#' @rdname set_get_global
#' @export
get_arcET_global <- function(which, default = NULL) {
  arcET_global[[which]] %||% default
}
